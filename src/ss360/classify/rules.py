...
def classify(
    ...
    return ("unknown", 0.1, all_reasons if all_reasons else ["no_classification_rules_matched"])


def _check_offline_expiry(match: str, kind: str) -> Tuple[FindingCategory, float, List[str]]:
...
def _check_test_markers(match: str, path: str, kind: str) -> Tuple[FindingCategory, float, List[str]]:
...
def _check_validator_signals(validation_results: List[Dict[str, Any]]) -> Tuple[FindingCategory, float, List[str]]:
...
def _extract_azure_sas_expiry(sas_url: str) -> Optional[datetime]:
    ...
    try:
        ...
        if 'se' in params:
            ...
            return datetime.fromisoformat(expiry_str.replace('Z', '+00:00')).replace(tzinfo=None)
    except Exception:
        pass
    return None
