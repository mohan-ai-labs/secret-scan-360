...
class PolicyEnforcer:
    ...
    def enforce(
            ...
            for waiver in get_active_waivers(self.config):
                if is_waiver_active(waiver, finding_path, rule_id):
                    waivers_applied.append({
                        "finding": f"{rule_id}:{finding_path}",
                        "waiver": waiver
                    })
                    waiver_found = True
                    break
        ...
        risk_violations = self._check_risk_score_violations(filtered_findings, validation_results)
    ...
    def _check_budget_violations(self, findings: List[Dict[str, Any]]) -> List[PolicyViolation]:
        """Check budget constraint violations."""
        violations = []
        budgets = self.config.get("budgets", {})
        
        # Check new findings budget
        max_new_findings = budgets.get("new_findings", 0)
        if len(findings) > max_new_findings:
    ...
    def format_violations_report(self, result: PolicyEnforcementResult) -> str:
        ...
        for violation in result.violations:
            ...
            report.append(f"{severity_icon} {violation.type.value}: {violation.message}")
