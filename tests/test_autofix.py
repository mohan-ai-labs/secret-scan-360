...
class TestAutofixPlanner:
    ...
    def test_github_pat_planning(self):
        ...
        remove_action = next(
            item for item in plan if item.action == ActionType.REMOVE_LITERAL
        )
        ...
        # Check revoke action
        revoke_action = next(
            item for item in plan if item.action == ActionType.REVOKE_TOKEN
        )
    ...
    def test_aws_key_planning(self):
        ...
        replace_action = next(
            item for item in plan if item.action == ActionType.REPLACE_WITH_SECRET_REF
        )
        assert "secretsmanager" in replace_action.replacement

        # Check deactivate action
        deactivate_action = next(
            item for item in plan if item.action == ActionType.DEACTIVATE_KEY
        )
