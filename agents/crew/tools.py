...
def commit_push_as_user(repo_dir: str, branch: str, message: str):
    name  = os.getenv("GIT_AUTHOR_NAME",  os.getenv("GIT_COMMITTER_NAME", "Mohan Krishna Alavala"))
    email = os.getenv("GIT_AUTHOR_EMAIL", os.getenv("GIT_COMMITTER_EMAIL", "you@example.com"))
    sh('git add -A', repo_dir)
    sh(f'git -c user.name="{name}" -c user.email="{email}" commit -m "{message}" || true', repo_dir)
...
def call_api_scan(api_base: str, repo_url: str, token: str | None = None, timeout=1800):
    ...
    r = requests.post(f"{api_base}/scan", headers=headers, json={"repo_url": repo_url}, timeout=timeout)
    r.raise_for_status()
    return r.json()
...
def write_report(repo_dir: str, result: dict):
    ...
    true_hits = result.get("true_hits", sum(1 for f in result.get("findings",[]) if f.get("is_secret")))
    total = len(result.get("findings", []))
    scan_id = result.get("scan_id", "N/A")
    lines = [f"# SS360 Scan Report",
    ...
    for f in result.get("findings", [])[:10]:
        lines.append(f"- `{f.get('path')}` — **{f.get('kind')}** — secret={f.get('is_secret')} — {f.get('reason','')[:120]}")
